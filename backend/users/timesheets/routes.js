...
module.exports = (app) => {
  ...
  app.put('/user/timesheets/:timesheetId/close', passport.authenticate('jwt', {session: false}), bodyParser.json(), (req, res) => controller.closeTimesheetByTimesheetId(req, res));
  app.post('/user/timesheets/:timesheetId/pdf', passport.authenticate('jwt', {session: false}), bodyParser.json(), (req, res) => controller.exportTimesheetAsPDFByTimesheetId(req, res));
